# -*- encoding: utf-8 -*-
# stub: gem_plugin 0.2.3 ruby lib

Gem::Specification.new do |s|
  s.name = "gem_plugin".freeze
  s.version = "0.2.3".freeze

  s.required_rubygems_version = Gem::Requirement.new(">= 0".freeze) if s.respond_to? :required_rubygems_version=
  s.require_paths = ["lib".freeze]
  s.authors = ["Zed A. Shaw".freeze]
  s.cert_chain = ["-----BEGIN CERTIFICATE-----\\ntbQaNYkU127B3l/VNpd8fQm3Jkl/PqCCmDBQjUszFrJEODug\n-----END CERTIFICATE-----\n".freeze, "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n".freeze]
  s.date = "2007-10-26"
  s.description = "A plugin system based on rubygems that uses dependencies only".freeze
  s.email = "".freeze
  s.executables = ["gpgen".freeze]
  s.files = ["bin/gpgen".freeze]
  s.homepage = "".freeze
  s.rubygems_version = "3.5.9".freeze
  s.summary = "A plugin system based on rubygems that uses dependencies only".freeze

  s.installed_by_version = "3.5.9".freeze if s.respond_to? :installed_by_version
end
